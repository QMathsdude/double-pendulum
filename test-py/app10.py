...
class DoublePendulumApp(tb.Frame):
    ...
    def create_graphs(self):
        ...
        (self.bob1,) = self.ax1.plot([], [], ms=5, mfc='crimson', mec='crimson', marker='o', ls='--', lw=0.5,color='maroon', zorder=2)
        ...
        self.ax2.set_aspect('auto', adjustable='box') # Default to auto, managed by box
        ...
        (self.phase1,) = self.ax3.plot([], [], ls='-', lw=1, color='crimson')
        (self.phase2,) = self.ax3.plot([], [], ls='-', lw=1, color='royalblue')
    ...
    def update_animation(self, frame):
        ...
        # --- Dynamic Y-Axis Resizing for ax2 (Angle Plot) ---
        max_angle = np.max(np.abs(np.concatenate([ang1, ang2])))
        current_ylim = self.ax2.get_ylim()
        current_y_limit = max(np.abs(current_ylim))
        buffer = 5.0 
        
        if max_angle + buffer > current_y_limit:
            new_limit = np.ceil((max_angle + buffer) / 10.0) * 10.0
            
            self.ax2.set_ylim(-new_limit, new_limit)
            self.ax2.autoscale_view(scalex=False, scaley=True) # Fix 1
            
            # 👇 FIX 2: Ensure the aspect remains set to auto-adjust by the subplot box size
            self.ax2.set_aspect('auto', adjustable='box') 
            
            # The most crucial step for layout consistency in Matplotlib backends:
            # Force a canvas and figure redraw to finalize the layout change.
